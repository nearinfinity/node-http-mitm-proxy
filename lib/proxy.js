...
Proxy.prototype._onHttpServerRequest = function(isSSL, clientToProxyRequest, proxyToClientResponse) {
  ...
  if (hostPort === null) {
	...
	const proxyPath = ctx.isSSL ? "https" : "http" + "://" + hostPort.host + ":" + ctx.clientToProxyRequest.url;

    ctx.proxyToServerRequestOptions = {
      method: ctx.clientToProxyRequest.method,
      path: this.proxy ? proxyPath : ctx.clientToProxyRequest.url,
      host: this.proxy ? this.proxy.host : hostPort.host,
      port: this.proxy ? this.proxy.port : hostPort.path,
      headers: headers,
      agent: ctx.isSSL ? self.httpsAgent : self.httpAgent
    };
