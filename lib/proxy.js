...
Proxy.prototype.listen = function(options, callback) {
  ...
  ca.create(this.sslCaDir, function(err, ca) {
    ...
    if (self.forceSNI) {
      ...
      self._createHttpsServer(Object.assign({}, self.httpsServerOptions), function(port, httpsServer, wssServer) {
        if (!self.silent) {
        ...
        self.httpsPort = port;
        if (options.handle) {
          self.httpServer.listen(options.handle, callback);
        } else {
...
Proxy.prototype._createHttpsServer = function (options, callback) {
  ...
  if (options.httpsHandle) {
      listenArgs.unshift(options.httpsHandle);
  } else {
    ...
    self.onCertificateRequired(hostname, function (err, files) {
      ...
      async.auto({
        ...
        if (self.forceSNI && !hostname.match(/^[\d\.]+$/)) {
          ...
          self._createHttpsServer(Object.assign({}, results.httpsOptions, self.httpsServerOptions), function(port, httpsServer, wssServer) {
            if (!self.silent) {
